"""Evaluation helpers for comparing predictions against gold data."""
...
class EvaluationConfig:
    """Configuration for evaluation runs against a gold set."""
...
def evaluate_against_gold(predictions: pd.DataFrame, config: EvaluationConfig) -> Dict[str, Any]:
    """Compare predictions against the configured gold set."""
    ...
    for pred_col, gold_col in label_columns:
        ...
        if len(gold) >= 30:
            ...
            results[pred_col.replace("_pred", "")]["confidence_interval"] = (
                max(0.0, accuracy - margin),
                min(1.0, accuracy + margin),
            )
...
def compare_against_history(
    ...
    """Compute drift statistics against historical runs."""
...
__all__ = ["EvaluationConfig", "evaluate_against_gold", "compare_against_history"]
